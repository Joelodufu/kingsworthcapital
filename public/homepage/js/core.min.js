...
/**
 * @module Custom Radial Progress Prototype
 * @author ATOM
 * @requires module:αUtil
 * @requires module:αCounter
 * @requires module:αProgressCircles
 * @see      {@link https://codepen.io/OXAYAZA/pen/omvaYW}
 */
function customRadialProgress(r){function t(r){this.params=Util.merge([this.defaults,r]),this.params.node.customProgress=this,this.init(),this.params.dividend.addEventListener("blur",this.run.bind(this)),this.params.divider.addEventListener("blur",this.run.bind(this))}return t.prototype.defaults={node:null,dividend:null,divider:null,bar:null,dot:null,duration:1e3,refresh:10},t.prototype.internal={dot:{rect:null,rx:0,ry:0,cx:0,cy:0}},t.prototype.init=function(){aCounter({node:this.params.dividend,duration:this.params.duration,refresh:this.params.refresh}),aCounter({node:this.params.divider,duration:this.params.duration,refresh:this.params.refresh}),aProgressCircle({node:this.params.bar}),this.internal.dot.rect=this.params.dot.getBoundingClientRect(),this.internal.dot.rx=this.params.bar.progressCircle.internal.viewBox[2]/2-this.internal.dot.rect.width/2,this.internal.dot.ry=this.params.bar.progressCircle.internal.viewBox[3]/2-this.internal.dot.rect.height/2},t.prototype.run=function(){this.params.dividend.counter.params.to=Number(this.params.dividend.textContent),this.params.divider.counter.params.to=Number(this.params.divider.textContent),this.params.dividend.counter.params.to<this.params.divider.counter.params.to?this.progress=this.params.dividend.counter.params.to/this.params.divider.counter.params.to:this.progress=this.params.divider.counter.params.to/this.params.dividend.counter.params.to,this.progress>1?this.progress=1:this.progress<-1&&(this.progress=-1),this.params.dividend.counter.run(),this.params.divider.counter.run(),this.easeCircle(360*this.progress)},t.prototype.easeCircle=function(){var r={value:0,loop:0};r.loops=this.params.duration/this.params.refresh,r.increment=this.progress/r.loops,r.id=setInterval(function(){r.loop++,r.value+=r.increment,r.loop>=r.loops?(this.params.bar.progressCircle.render(360*this.progress),clearInterval(r.id)):this.params.bar.progressCircle.render(360*r.value),this.params.dot&&this.updDot()}.bind(this),this.params.refresh),t.prototype.updDot=function(){this.internal.dot.cx=Math.sin(this.params.bar.progressCircle.params.angle*Math.PI/180)*this.internal.dot.rx+this.params.bar.progressCircle.internal.ellipse.cx,this.internal.dot.cy=-Math.cos(this.params.bar.progressCircle.params.angle*Math.PI/180)*this.internal.dot.ry+this.params.bar.progressCircle.internal.ellipse.cy,this.params.dot.setAttribute("cx",this.internal.dot.cx),this.params.dot.setAttribute("cy",this.internal.dot.cy)}},new t(r)}

/**
 * @module Custom Linear Progress Prototype
 * @author ATOM
 * @requires module:αUtil
 * @requires module:αCounter
 * @requires module:αProgressCircles
 * @see      {@link https://codepen.io/OXAYAZA/pen/omvaYW}
 */
function customLinearProgress(r){function s(r){this.params=Util.merge([this.defaults,r]),this.params.node.customProgress=this,this.init(),this.params.dividend.addEventListener("blur",this.run.bind(this)),this.params.divider.addEventListener("blur",this.run.bind(this))}return s.prototype.defaults={node:null,dividend:null,divider:null,bar:null,duration:1e3,refresh:10},s.prototype.init=function(){aCounter({node:this.params.dividend,duration:this.params.duration,refresh:this.params.refresh}),aCounter({node:this.params.divider,duration:this.params.duration,refresh:this.params.refresh})},s.prototype.run=function(){this.params.dividend.counter.params.to=Number(this.params.dividend.textContent),this.params.divider.counter.params.to=Number(this.params.divider.textContent),this.params.dividend.counter.params.to<this.params.divider.counter.params.to?this.progress=this.params.dividend.counter.params.to/this.params.divider.counter.params.to:this.progress=this.params.divider.counter.params.to/this.params.dividend.counter.params.to,this.progress>1?this.progress=1:this.progress<-1&&(this.progress=-1),this.params.dividend.counter.run(),this.params.divider.counter.run(),this.params.bar.style.width=100*this.progress+"%"},new s(r)}
 ...
 */
function donutChart(){var t=d3.format(".4r"),a=d3.format(",.0%"),n=d3.scaleOrdinal(d3.schemeCategory20c),e={width:null,height:null,thickness:.2,variable:null,category:null,fill:null,padAngle:null,cornerRadius:null};function r(r){r.each(function(l){var i=Math.min(e.width,e.height)/2,d=d3.pie().value(function(a){return t(a[e.variable])}).sort(null),c=d3.arc().outerRadius(.8*i).innerRadius(i*(.8-e.thickness)).cornerRadius(e.cornerRadius).padAngle(e.padAngle),s=d3.arc().outerRadius(.9*i).innerRadius(.9*i),u=r.append("svg").attr("class","donut-chart-svg").attr("width",e.width).attr("height",e.height).attr("viewBox",[0,0,e.width,e.height].join(" ")).append("g").attr("transform","translate("+e.width/2+","+e.height/2+")");r.append("div").attr("class","donut-chart-labels"),u.append("g").attr("class","donut-chart-slices"),u.append("g").attr("class","donut-chart-lines");u.select(".donut-chart-slices").datum(l).selectAll("path").data(d).enter().append("path").attr("class","donut-chart-slice").attr("fill",function(t){return t.data[e.fill]?t.data[e.fill]:n(t.data[e.category])}).attr("d",c),r.select(".donut-chart-labels").selectAll("div").data(d).enter().append("div").attr("class",function(t){return"donut-chart-label "+(h(t)<Math.PI?"donut-chart-label-right":"donut-chart-label-left")}).html(function(t){return['<span class="donut-chart-title">',t.data[e.category],"</span>",' <span class="donut-chart-percentage">',a(t.data[e.variable]),"</span>"].join("")}).style("left",function(t){var a=s.centroid(t);return a[0]=.95*i*(h(t)<Math.PI?1:-1),a[0]=(e.width/2+a[0])/e.width*100,a[0]+"%"}).style("top",function(t){var a=s.centroid(t);return a[1]=(e.height/2+a[1])/e.height*100,a[1]+"%"});r.select(".donut-chart-labels").style("width",function(){return e.width+"px"}).style("padding-top",function(){return e.height/e.width*100+"%"});u.select(".donut-chart-lines").selectAll("polyline").data(d).enter().append("polyline").attr("class","donut-chart-line").attr("points",function(t){var a=s.centroid(t);return a[0]=.95*i*(h(t)<Math.PI?1:-1),[c.centroid(t),s.centroid(t),a]});function h(t){return t.startAngle+(t.endAngle-t.startAngle)/2}})}for(var l in e)r[l]=function(t){return arguments.length?(e[this]=t,r):e[this]}.bind(l);return r}
